...
def set_cell_text(cell, text, font_name='Verdana', font_size=10, alignment='center', vertical_alignment='center'):
    ...
    for border_name in ['top', 'left', 'bottom', 'right']:
        ...
        border.set(qn('w:sz'), '8')  # Border size
...
def substitute_in_footer_full(directory, old_text, new_text):
    for root, dirs, files in os.walk(directory):
        # Skip the "Obsoletos" folder
        if "Obsoletos" in dirs:
            dirs.remove("Obsoletos")
...
# old_text = ' USO INTERNO '
# new_text = ' CONFIDENCIAL '
# substitute_in_footer_full(directory_path, old_text, new_text)
